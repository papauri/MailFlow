...
import { MailMinus, ShieldCheck, Search, Loader2, Skull, X, Undo2, CheckCircle2, Trash2, Filter, Tag, Archive, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-150">
        ...
        {/* Toast / Notification */}
        {purgeToast && (
          <div className="bg-slate-900 text-white px-4 py-2.5 flex items-center justify-between text-xs font-semibold animate-in slide-in-from-top-2 duration-150">
            ...
            {purgeToast.undoItem && (
              <button
                ...
                className="flex items-center gap-1 bg-white/15 hover:bg-white/25 px-2.5 py-1 rounded-md text-[11px] font-bold text-white transition-colors cursor-pointer"
              >
          ...
          <div className="flex items-center gap-2 self-end sm:self-center">
            {activeTab === 'active' && !aiAnalysis && subscriptions.length > 0 && (
              <button
                ...
                disabled={analyzing}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-lg text-xs font-semibold transition-all shadow-xs disabled:opacity-50"
              >
                  ...
                  <div className="flex items-center justify-between mb-4 bg-slate-50 p-3 rounded-xl border border-slate-200">
                    ...
                    {selectedLogs.size > 0 && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 font-semibold text-xs sm:text-sm rounded-lg transition-colors border border-red-200 cursor-pointer"
                      >
                        ...
                        <li key={log.id} className={cn("p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50 transition-colors", isProcessing && "opacity-50 pointer-events-none")}>
                          ...
                          {log.trashedIds && log.trashedIds.length > 0 && (
                            <button
                              ...
                              className="px-4 py-2 bg-white border border-slate-300 hover:bg-slate-50 hover:border-slate-400 text-slate-700 text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-1.5 shrink-0 w-full sm:w-auto shadow-2xs cursor-pointer"
                            >
